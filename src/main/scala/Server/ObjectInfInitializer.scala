...
object ObjectInfInitializer {
  ...
  //ToDo Ops
  val ops = Ops(wrt1req, resWrt1req, Map(), Option.empty)

  //ToDo informazioni oggetto 2
  val objInf2 = ObjectInformation(certificate2, Option.empty, Option(ops), Option(oldOps), 5)
  ...
  val objects = Objects(Map(1 -> obj1, 2 -> obj2))

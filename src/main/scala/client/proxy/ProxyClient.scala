...
object ProxyClient{
  ...
  private class ProxyClientActor extends ProxyClient(){
    ...
    /*TODO
    1. I nomi dei client non ci fregano direttamente visto che possiamo memorizzare gli actor ref remoti (ma bisogna fa attenzione)
    2. Gli actor ref remoti dei server come li facciamo a conoscere a priori? bho
     */
    private var opCounter = 0

    override def write[T](op: T => T, oid: String): T = require(RequireWriteMessage(oid, op, opCounter))

    override def read[T](oid: String): T = require(RequireReadMessage(oid, opCounter))
